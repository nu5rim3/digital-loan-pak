...
export const setAxiosToken = async (token: string, idx: string) => {
  if (token) {
    ...
    axiosInstance.defaults.headers.common["Content-Security-Policy"] =
      "default-src 'self' http: https: data: blob: 'unsafe-inline'";
    axiosInstance.defaults.headers.common["X-Frame-Options"] = "SAMEORIGIN";
    axiosInstance.defaults.headers.common["Strict-Transport-Security"] =
