...
const BnsOrAgriExpenses: React.FC = () => {
    ...
    const { cashFlows, cashFlowsLoading, bnsOrAgriExpenses, addBnsOrAgriExpenses, updateBnsOrAgriExpenses, fetchBnsOrAgriExpenses, removeBnsOrAgriExpenses,
        calculateTaxableAmount, calculateTotalBusinessExpense, calculateTotalExpense, calculateTotalHouseholdExpense, calculateNetMonthlyDisposable,
        calculateAnnualDisposable, calculateAnnualHousehold, calculateAnnualRevenue, calculateMaxDebtBurden, checkAlegibleFroLoan, fetchCashFlows,
        calucalteMaxLoanValue } = useCreditStore()
    ...
    useEffect(() => {
        if (cashFlows) {
            addBnsOrAgriExpenses(cashFlows?.bnsOrAgriExpenses ?? [])
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [cashFlows])
    ...
    const onSubmit = (data: IFinancialEntry) => {
        if (mode === 'update') {
            updateBnsOrAgriExpenses(selectedDetail?.key ?? '', data).finally(closeModal);
        } else if (mode === 'save') {
            addBnsOrAgriExpenses(data).finally(closeModal);
        } else if (mode === 'remove') {
            removeBnsOrAgriExpenses(selectedDetail?.key ?? '').finally(closeModal);
        }
    };

    useEffect(() => {
        fetchBnsOrAgriExpenses()
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [addBnsOrAgriExpenses, bnsOrAgriExpenses])
...
export default BnsOrAgriExpenses