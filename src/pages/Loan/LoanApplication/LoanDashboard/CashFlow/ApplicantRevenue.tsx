...
const ApplicantRevenue: React.FC = () => {
    ...
    const onSubmit = (data: IFinancialEntry) => {
        ...
        if (mode === 'update') {
            ...
            addApplicantRevenue(data).finally(closeModal);
        } else if (mode === 'remove') {
            removeApplicantRevenue(selectedDetail?.key ?? '').finally(closeModal);
        }
    };
    ...
    useEffect(() => {
        if (cashFlows) {
            addApplicantRevenue(cashFlows?.applicantRevenue ?? [])
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [cashFlows])
            ...
            >
                ...
                >
                    <div className='grid grid-cols-4 gap-2'>
                        <Form.Item label="Revenue" validateStatus={errors.key ? 'error' : ''} help={errors.key?.message} required>
                            <Controller
                                name="key"
                                control={control}
                                render={({ field }) => (
...
const DetailsCard: React.FC<{ detail: IFinancialEntry; onEdit: () => void; onRemove: () => void; loading: boolean; }> = ({ detail, loading, onEdit, onRemove }) => (
    <Card loading={loading}>
        ...
        <Descriptions column={1}>
            <Descriptions.Item label="Revenue">{detail.key}</Descriptions.Item>
...
export default ApplicantRevenue