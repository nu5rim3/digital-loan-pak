...
const BusinessIntroducerPage: React.FC = () => {
  ...
  const confirmAction = (record: any, status: 'A' | 'I') => {
    confirm({
      title: `Are you sure you want to ${status === 'A' ? 'activate' : 'deactivate'} this record?`,
      onOk: async () => {
        try {
          ...
          message.success(`Record ${status === 'A' ? 'activated' : 'deactivated'} successfully`);
          fetchData(page);
        } catch {
  ...
  const columns = [
    ...
    },
    {
      title: 'Status',
      dataIndex: 'status',
      render: (status: string) => status === 'A' ? <Tag color="green">ACTIVE</Tag> : <Tag color="red">INACTIVE</Tag>,
    },
    // {
    //   title: 'Action',
  ...
  return (
    <div className="p-2">
      <div className="flex justify-between items-center mb-4">
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditRecord(null); setShowCreateModal(true); }}>Create Business Introducers</Button>
      </div>

      <Card>
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <Select value={searchBy} onChange={setSearchBy} style={{ width: 180 }}>
            {searchFields.map((f) => (
              <Option key={f.value} value={f.value}>{f.label}</Option>
            ))}
          </Select>
          <Input placeholder="Search..." value={searchValue} onChange={(e) => setSearchValue(e.target.value)} style={{ width: 250 }} />
          <Button icon={<SearchOutlined />} onClick={handleSearch} type="primary">Search</Button>
          <Button icon={<ReloadOutlined />} onClick={handleReset}>Reset</Button>
        </div>

        <Table rowKey="empNo" columns={columns} dataSource={data} pagination={{ current: page + 1, total, pageSize: PAGE_SIZE, onChange: (p) => setPage(p - 1), showSizeChanger: false }} loading={loading} />
      </Card>

      <CreateModal visible={showCreateModal} onClose={() => { setShowCreateModal(false); setEditRecord(null); }} onCreate={handleCreate} editRecord={editRecord} />
    </div>
  );
};
